...
export function createItems<T>(count: number, factory: () => T): T[] {
  return new Array(count).fill({}).map(() => factory());
}

export function createUniqueItems<T extends Record<string, any>>(
  arr: T[],
  ...uniqueIdentifers: Array<keyof T>
): T[];
export function createUniqueItems<T>(arr: T[]): T[];
export function createUniqueItems<T>(
  arr: T[],
